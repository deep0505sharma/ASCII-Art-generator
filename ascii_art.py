...
for i in range(num_rows):
    min_h = min(int(i+1)*ceil_h,height)
    row_pix = int(i*ceil_h)
    ...
    line = "".join([char_list[min(int(np.mean(image[row_pix:min_h,int(j*ceil_w):min(int((j+1)*ceil_w),width)])/225*num_chars-1))]
